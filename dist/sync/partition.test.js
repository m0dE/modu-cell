...
import { computePartitionAssignment, computePartitionCount, computePartitionSeed, weightedRandomPick, getEntityPartition, isClientAssigned, getClientPartitions, computeDegradationTier } from './partition';
...
describe('weightedRandomPick', () => {
    test('returns correct number of picks', () => {
        const picks = weightedRandomPick(['a', 'b', 'c', 'd'], 2, 42, { a: 100, b: 100, c: 100, d: 100 });
        expect(picks).toHaveLength(2);
    });
    test('never picks same client twice', () => {
        for (let seed = 0; seed < 100; seed++) {
            const picks = weightedRandomPick(['a', 'b', 'c'], 2, seed, { a: 100, b: 100, c: 100 });
            expect(new Set(picks).size).toBe(picks.length);
        }
    });
    test('returns all clients when count >= clients.length', () => {
        const picks = weightedRandomPick(['a', 'b', 'c'], 5, 42, { a: 100, b: 100, c: 100 });
    ...
    test('favors higher reliability clients statistically', () => {
        ...
        for (let seed = 0; seed < 1000; seed++) {
            const picks = weightedRandomPick(['high', 'low'], 1, seed, { high: 100, low: 10 });
            counts[picks[0]]++;
        }
